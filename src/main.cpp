...
void print_conf(const ServerConfig &conf) {
    ...
    for (size_t i = 0; i < locations.size(); i++) {
        ...
        if (loc.getReturnDir().second.empty() && loc.getReturnDir().first != 0)
            ...
            std::cout << "  Return Directory: " << loc.getReturnDir().first << " " << loc.getReturnDir().second
                      << std::endl;
...
ServerConfig init_config(char *path) {
    return ParseConfig(path).getConfig();
}
...
int main(int argc, char **argv) {
    ...
    try {
        ServerConfig config = init_config(argv[1]);
        print_conf(config);
